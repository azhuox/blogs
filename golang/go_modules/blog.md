...
The following is the contents of the `go.mod` file automatically generated by Go. You can see that the `go.mod` file defines the module's path, glues anything under the `path/to/example/module` directory as a single unit and lists all of its dependencies.
